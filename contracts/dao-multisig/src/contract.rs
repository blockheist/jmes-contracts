...
const CONTRACT_NAME: &str = "dao-multisig";
...
pub fn execute_propose(
    ...
    Ok(Response::new()
        .set_data(to_binary(&ProposeResponse { proposal_id: id })?)
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        dao_members_addr: config.group_addr.addr(),
    })
}
...
mod tests {
    ...
    fn test_vote_works() {
        ...
        let vote: VoteResponse = app
            .wrap()
            .query_wasm_smart(&flex_addr, &QueryMsg::GetVote { proposal_id, voter })
            .unwrap();
        assert_eq!(
            vote.vote.unwrap(),
            VoteInfo {
